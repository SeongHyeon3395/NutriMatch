-- Ensure chat token RPC functions exist, including a no-argument wrapper for RPC callers.
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'app_users_plan_id_check'
  ) then
...
create or replace function public.get_monthly_chat_token_status()
returns table (
  month text,
  plan_id text,
  used integer,
...
as $$
  select *
  from public.get_monthly_chat_token_status(null::text);
$$;
...
grant execute on function public.get_monthly_chat_token_status() to authenticated;
